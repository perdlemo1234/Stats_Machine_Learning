...
library(glmnet)
library(readr)
Salary_Data <- read_csv("~/Stats_Machine_Learning/CW3/Salary_Data.csv")
rowSums(is.na(Salary_Data))

# Define split sizes
train_size <- floor(0.6 * nrow(Salary_Data))  
val_size <- floor(0.2 * nrow(Salary_Data))    
...
TPE_algo <- function(train_data, valid_data, max_iter=100) {
  ...
  for (iter in max_iter){
    ...
    #Skip iteration if fewer than 2 values exist in either froups 
    ...
    lambda_star <- sample_lambdas[which.min(ratio)]
